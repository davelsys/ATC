...
Partial Class Order
    ...
    Private varCarrierPin As String = vbNullString
    ...
    Private varMinutesUsed As String = vbNullString
    ...
    Private varCashStackedCount As Integer = 0
    ...
    Private varPaymentsExists As Boolean = False
    ...
    Private Sub InitializePage()
        ...
        Else
            ...
            BindTransactionHistoryGrid() 'SG 03/07/17 moved this up 

            PopulateAllPlanDropdowns()
            PopulateRenewals() 'SG 01/26/17 moved it down
            SetSaveNoteBtnProps()
            ...
            'PopulateAllPlanDropdowns() 'SG 01/05/17 moved this down to be after the BindTransactionHistoryGrid() function to set the varPaymentsExist variable
            'PopulateRenewals()
        End If
    ...
    Private Sub SetupVendorPinField()
        If _isNewOrder Then
            ...
            'portPhone.Attributes("onchange") = "isPortNumberUnique();"
        Else
    ...
    Private Sub PopulateAllPlanDropdowns()
        ...
        Dim sql As String = "SELECT [planid], [planname] FROM [Plans] WHERE planref = 'Monthly' AND [carrier] = @carrier and isnull(active, 0) = 1 " 'ORDER BY [minutesprovided]"
        If varPaymentsExists = False Then 'And varCarrierName.ToLower <> "vzpp" Then 'SG 04/06/17
            sql = sql & " union select [planid], 'AutoPay' + planname from plans where planref = 'Monthly' AND [carrier] = @carrier and autopay_cost is not null and isnull(active, 1) = 1"
        End If
        sql = sql & " order by planname "
        ...
        sql = "SELECT [planid], [planname] FROM [Plans] WHERE planref = 'Pay As You Go' AND [carrier] = @carrier and isnull(active, 0) = 1"
        'If varPaymentsExists = False Then ' SG 01/05/17 not applicable for cash
        '    sql = sql & " union select [planid], 'AutoPay' + planname from plans where planref = 'Pay As You Go' AND [carrier] = @carrier and autopay_cost is not null and isnull(active, 1) = 1"
        'End If
        cmd = New SqlCommand(sql, connection)
        cmd.Parameters.Add("@carrier", SqlDbType.VarChar).Value = varCarrierName
    ...
    Private Sub DisableTabs()
        ...
        If Session("Userlevel") <> 1 Then 'SG 12/21/16 as per Mr. Greenberg
            carrierName.Items.FindByText("Verizon").Enabled = False
        End If
    End Sub
    ...
    Protected Sub adminTasksRadiolist_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles adminTasksRadiolist.Load
        ...
        If Not _isNewOrder Then
            ...
            Try
                If userLevel <> 1 Then 'Admin
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("refund")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("convertedToCON")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("convertTELCO")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("convertPP")))
                    'SG removed all converts from anyone that is not admin 1/16/17
                End If
                ...
                ElseIf varCarrierName.ToLower = "page plus" Then
                    ...
                    'adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("ATprovision")))
                    'adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("newVER")))
                    'adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("convertTELCO"))) 'SG removed the option from pp 12/16/16
                ...
                ElseIf varCarrierName.ToLower = "concord" Then
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("convertedToCON")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("updateStack")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("removeStack")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("disconnect")))
                ...
                ElseIf varCarrierName.ToLower = "sprint" Then
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("convertedToSprint")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("updateStack")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("removeStack")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("disconnect")))

                ElseIf varCarrierName.ToLower = "telco" Then
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("convertTELCO")))
                    adminTasksRadiolist.Items().RemoveAt(adminTasksRadiolist.Items().IndexOf(adminTasksRadiolist.Items().FindByValue("convertPP")))

                ElseIf varCarrierName.ToLower = "vzpp" Then
    ...
    Protected Sub moniterChckBox_Load(sender As Object, e As System.EventArgs) Handles moniterChckBox.Load
        ...
        If Not _isNewOrder Then
            ...
            ElseIf varCarrierName.ToLower = "verizon" Or varCarrierName.ToLower = "telco" Then
                moniterChckBox.Visible = False
            ElseIf varCarrierName.ToLower = "vzpp" Then
                vzppPlanDatesDiv.Visible = True
                moniterChckBox.Visible = False

                'tbVZPPPlanStart.Text = DateTime.Now.ToShortDateString
                'tbVZPPPlanExp.Text = DateTime.Now.AddMonths(1).ToShortDateString
    ...
    Protected Sub verRegisterGv_Load(sender As Object, e As System.EventArgs) Handles verRegisterGv.Load
        ...
        If Not _isNewOrder Then
            ...
            ElseIf varCarrierName.ToLower = "telco" Then
                ...
                sql.Append("MonthExpDate[Expires], p.planname AS [Plan], isnull(MinutesAvailable,0),[MinutesAvailable], ")
                sql.Append(" 0 [Used],1.0  AS Fee, 99 [Available],PlanExpDate [Completed] ")
                sql.Append(" FROM TelcoMDN m JOIN Plans p ON m.Planid = p.planid WHERE MID = @CellNum ORDER BY [Renewed] DESC; ")
    ...
    Private Sub GetVerData()
        ...
        ' MDN table
        sql.Append("[Status], CashBalance [Balance], CAST( [PlanExpDate] AS DATE ) AS [PlanExpDate], ")
        sql.Append("CAST( MonthExpDate AS DATE ) AS [MonthExpDate], ")
        ...
        sql.Append("MinutesUpdated [LastStatus], [Stacked], [cashstacked],")
        ...
        If reader.HasRows Then
            ...
            varExpirationDate = ReplaceDBNull(reader.Item("PlanExpDate"), Nothing)
            varPlanExpirationDate = ReplaceDBNull(reader.Item("MonthExpDate"), Nothing)
            ...
            varCashStackedCount = ReplaceDBNull(reader.Item("cashstacked"), 0)
    ...
    Private Sub GetVZPPData()
        ...
        sql.Append("CAST( MonthExpDate AS DATE ) AS [MonthExpDate], ")
        sql.Append(" [MinutesinPlan], ")
        sql.Append(" minutesavailable AS [min_remain], ")
        sql.Append("MinutesUpdated [LastStatus], [Stacked], [cashstacked],")
        sql.Append("MinutesAvailable [TotalMinutes], ")
        ...
        sql.Append("[BalAvailable] AS intl_bal ")
        sql.Append("FROM [orders] ")
        sql.Append("LEFT JOIN [ppc].[dbo].[VZPPMDN] ON [VZPPMDN].[MID] = [orders].[cell_num] ")
        sql.Append("LEFT JOIN Plans ON [orders].[plan_id] = [Plans].[planid] ")
        sql.Append("LEFT JOIN [ATCIntl].[dbo].[IntlBalances] ON orders.[intl_pin] = [IntlBalances].[IntlPin] ")
        ...
        If reader.HasRows Then
            ...
            varSerialNumber = ReplaceDBNull(reader.Item("serial_num"), "")
            varVendorPin = ReplaceDBNull(reader.Item("vendor_pin"), "")
            varCarrierPin = ReplaceDBNull(reader.Item("carrier_pin"), "")
            varMonitor = ReplaceDBNull(reader.Item("monitor"), Session("UserMonitor"))
            varRenewalChargeType = ReplaceDBNull(reader.Item("renew_charge_type"), 0)
            varIsOrderClosed = ReplaceDBNull(reader.Item("order_closed"), False)
            varPlanID = ReplaceDBNull(reader.Item("plan_id"), 0)
            ...
            varIntlBalance = ReplaceDBNull(reader.Item("intl_bal"), 0)
            varVerStatusMsg = ReplaceDBNull(reader.Item("statusmsg"), "")
            varMinutesUsed = ReplaceDBNull(reader.Item("min_remain"), Nothing)
            If (reader("MinutesInPlan") Is DBNull.Value) Then
                ' Response.Write("minutes are null")
                varMinutesUsed = ReplaceDBNull(reader.Item("min_remain"), Nothing)
    ...
    Private Sub GetConcordData()
        ...
        sql.Append("CAST( MonthExpDate AS DATE ) AS [MonthExpDate], ")
        sql.Append(" [MinutesinPlan], ")
        sql.Append(" case when conmdn.planid = 30 then (5000 - minutesavailable) else minutesavailable end AS [min_remain], ")
        sql.Append("MinutesUpdated [LastStatus], [Stacked], [cashstacked],")
        sql.Append("MinutesAvailable [TotalMinutes], ")
        ' VerRegister
        'sql.Append("CAST( PlanExpDate AS DATE ) AS [PlanExpDate], ")
        ...
        sql.Append("[BalAvailable] AS intl_bal ")
        sql.Append("FROM [orders] ")
        sql.Append("LEFT JOIN [ppc].[dbo].[CONMDN] ON [CONMDN].[MID] = [orders].[cell_num] ")
        sql.Append("LEFT JOIN Plans ON [orders].[plan_id] = [Plans].[planid] ")
        sql.Append("LEFT JOIN [ATCIntl].[dbo].[IntlBalances] ON orders.[intl_pin] = [IntlBalances].[IntlPin] ")
        'sql.Append("LEFT JOIN ( ")
        'sql.Append("SELECT TOP (1) * FROM VerRegister WHERE Effective IS NOT NULL AND Completed IS NULL ")
    ...
    Private Sub GetTelcoData()
        ...
        sql.Append("FROM [orders] ")
        sql.Append("LEFT JOIN [ppc].[dbo].[TelcoMDN] ON [TelcoMDN].[MID] = [orders].[cell_num] ")
        sql.Append("LEFT JOIN Plans ON [orders].[plan_id] = [Plans].[planid] ")
        sql.Append("LEFT JOIN [ATCIntl].[dbo].[IntlBalances] ON orders.[intl_pin] = [IntlBalances].[IntlPin] ")
        ...
        reader.Close()
        con.Close()
        'Response.Write(varIsMonthlyRenew)
    End Sub
    ...
    Private Sub GetSprintData()
        ...
        sql.Append("[BalAvailable] AS intl_bal ")
        sql.Append("FROM [orders] ")
        sql.Append("LEFT JOIN [ppc_sprint].[dbo].[sprintMDN] ON [sprintMDN].[MID] = [orders].[cell_num] ")
        sql.Append("LEFT JOIN Plans ON [orders].[plan_id] = [Plans].[planid] ")
        sql.Append("LEFT JOIN [ATCIntl].[dbo].[IntlBalances] ON orders.[intl_pin] = [IntlBalances].[IntlPin] ")
        ...
        If varOrderCellNumber.Length = 0 Then
            checkForPayments(varOrderID)
        End If
    ...
    Private Sub CheckForPayments(ByVal varOrderID As Integer)

        Dim con2 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings(_ppcConStr).ConnectionString)
        con2.Open()

        Dim cmd2 As New SqlCommand
        cmd2.Connection = con2

        cmd2.CommandText = "if exists (select authtransid from authtrans where orderid = @orderid and ISNULL(cell_num,'') = '' and charged =1  and isnull(month_plan_id, cash_plan_id) in ("
        cmd2.CommandText = cmd2.CommandText & " select planid from plans where carrier = 'sprint') ) begin select 1 end else begin select 0 end "
        cmd2.Parameters.Add("@orderid", SqlDbType.Int).Value = varOrderID

        Dim hasPayment As Boolean = CBool(cmd2.ExecuteScalar())
    ...
    Private Function GetCostForCharge(ByVal planId As Integer, ByVal isAutoPay As Boolean) As Decimal
        connection.Open()
        Dim sql As String

        If isAutoPay Then
            sql = "SELECT [planref], [autoPay_cost] as [monthly_cost] FROM [Plans] WHERE [planid] = @PlanId"
        Else
            sql = "SELECT [planref], [plan_cost], [monthly_cost] FROM [Plans] WHERE [planid] = @PlanId"
        End If
    ...
    Private Sub SetOrderInfoBar()
        ...
        If varCarrierName.ToLower = "telco" Then 'SG 01/16/17 removed cash balance from telco as per Mr. Greenberg
            displayCashBalance.Visible = False
        Else
        ...
        If varIsMonthlyRenew Then 'FK added this check on 5/1/14 because with all talk, the monthly plan id gets switched on every payment, even if there is no renewal.  So this check makes sure that the price for that monthly_plan_id is shown in the infobar by renewals only if there really is a monthly-renewal set up
            renewalMonthLbl.Text = FormatCurrency(GetPlanCost(varRenewalMonthlyId, False, True), 2)
        Else
            renewalMonthLbl.Text = FormatCurrency(0, 2, True)
        End If
        ...
        minutesUsedLbl.Text = varMinutesUsed
        If varCarrierName.ToLower = "page plus" Or varCarrierName.ToLower = "vzpp" Then 'Or varCarrierName.ToLower = "all talk" 
            esnValidLbl.Visible = True
            esnLbl.Visible = True
            esnLbl.Visible = True
            esnLkBn.Visible = True
            planexpdiv.Visible = True
            divMinRemaining.Visible = True
        ...
        ElseIf varCarrierName.ToLower = "concord" Then
            ...
            If varPlanName = "Concord Unlim" Then
                divMinUsed.Visible = True
            Else
                divMinRemaining.Visible = True
            End If
        ...
        If varCarrierName.ToLower = "verizon" Or varCarrierName.ToLower = "telco" Then
            divMinRemaining.Visible = True
            cashStackedDiv.Visible = True
            cashStackedLbl.Text = varCashStackedCount
    ...
    Private Sub PopulateOrderForm()
        ...
        If varCarrierName.ToLower = "vzpp" Then
            tbVZPPPlanStart.Text = If(varExpirationDate IsNot Nothing, DateTime.Parse(varExpirationDate).ToShortDateString(), "")
            vzppVendorPin.Text = varVendorPin
            vzppVendorPW.Text = varCarrierPin
            'tbVZPPPlanExp.Text = If(varExpirationDate IsNot Nothing, DateTime.Parse(varExpirationDate).ToShortDateString(), "")
        End If
    ...
    Private Sub SetInvoiceAmounts()
        ...
        'SG 01/05/17 if monthly on autopay - get autopay price does not apply for cash
        Dim autoPay_Monthly As Boolean = If(monthlyPlanDropdown.SelectedItem.Text.IndexOf("auto") > -1, True, False)
        'Dim autoPay_Cash As Boolean = If(cashBalanceDropdown.SelectedItem.Text.IndexOf("auto") > -1, True, False)

        monthlyPlanDropdownCostLbl.Text =
         If(monthlyPlanDropdown.SelectedIndex > 0, FormatCurrency(GetCostForCharge(monthlyPlanDropdown.SelectedValue, autoPay_Monthly), 2), "")

        cashBalanceDropdownCostLbl.Text =
         If(cashBalanceDropdown.SelectedIndex > 0,
            FormatCurrency(GetCostForCharge(cashBalanceDropdown.SelectedValue, False), 2), "")
    ...
    Private Sub BindTransactionHistoryGrid()
        ...
        If transactionHistoryGridView.Rows.Count > 0 Then
            varPaymentsExists = True
        End If
        connection.Close()
    ...
    Private Sub GetTelcoCdr()
        ...
        sql.Append("SELECT [RECORD ID][REC ID], [BILLING NUMBER][MID], [CALLTIME] ,[FROM CITY]  + ' ' + [FROM STATE] [ORIGIN], [OTHER PARTY NUMBER][CALLING NUM]")
        sql.Append(", [TO CITY]  + ' ' + [TO STATE] [DEST],[TOTAL SECONDS OF CALL] [DUR],[CALLDURMIN][MIN] ")
        sql.Append("FROM [Telco].[dbo].[CDR_NEW] vcdr ")
        sql.Append("WHERE [BILLING NUMBER] = @CellNum ")
        sql.Append("AND [CALLTIME] BETWEEN @Start AND @End ")
        'sql.Append("ORDER BY [SEQID] DESC ")

        sql.Append("union SELECT [RECORD ID][REC ID], [BILLING NUMBER][MID], [CALLTIME] ,[FROM CITY]  + ' ' + [FROM STATE] [ORIGIN], [OTHER PARTY NUMBER][CALLING NUM]")
        sql.Append(", [TO CITY]  + ' ' + [TO STATE] [DEST],[TOTAL SECONDS OF CALL] [DUR],[CALLDURMIN][MIN] ")
        sql.Append("FROM [verizon].[dbo].[CDR_NEW] vcdr ")
        sql.Append("WHERE [BILLING NUMBER] = @CellNum ")
        sql.Append("AND [CALLTIME] BETWEEN @Start AND @End ")
        sql.Append("ORDER BY [CALLTIME] DESC ")

        ' Get cycle dates and total minutes
        sql.Append("SELECT @Start AS Start, @End AS [End], isnull(SUM( CALLDURMIN ), 0) + ")
        sql.Append(" (select isnull(sum(calldurmin),0) from verizon.dbo.cdr_new where [BILLING NUMBER] = @CellNum AND [CALLTIME] BETWEEN @Start AND @End) ")
        sql.Append(" AS TotalMinutes FROM [Telco].[dbo].CDR_NEW ")
        sql.Append("WHERE [BILLING NUMBER] = @CellNum  AND [CALLTIME] BETWEEN @Start AND @End; ")

        'Response.Write(sql.ToString)
        'Response.Write("<br /> 1- " & hdnNumOfCallDetailsIntervals.Value & "<br /> 2-" & callDetailFromIndex.Value)
        'Response.Write("<br />3- " & hdnNumOfCallDetailsIntervals.Value & "<br /> 4-" & callDetailEndIndex.Value)
        'Response.End()
        ...
        cmd.Parameters.Add("@CellNum", SqlDbType.VarChar).Value = varOrderCellNumber
        'ClientScript.RegisterStartupScript(Me.GetType(), "", "<script>alert('" & varCarrierName & "');</script>")
        If varCarrierName.ToLower = "telco" Then
            cmd.Parameters.Add("@sIndex", SqlDbType.Int).Value = (Convert.ToInt32(hdnNumOfCallDetailsIntervals.Value - callDetailFromIndex.Value))
            cmd.Parameters.Add("@eIndex", SqlDbType.Int).Value = (Convert.ToInt32(hdnNumOfCallDetailsIntervals.Value - callDetailEndIndex.Value))
        End If
    ...
    Private Sub BindIntlCallsGridView()
        ...
        sql.Append("([DialedTo]) AS DialedTo, ")
        sql.Append("[CallCost], [Duration], [DNIS], [Retrieved] FROM [IntlCalls] ")
    ...
    Private Function GetPlanCost(ByVal planId As Integer, Optional ByVal isAjax As Boolean = False, Optional ByVal AutoPayPrice As Boolean = False) As Decimal
        ...
        If AutoPayPrice = True And (varCarrierName = "Telco" Or varCarrierName = "Concord" Or varCarrierName.ToLower = "vzpp") Then
            sql = "SELECT [planref], [plan_cost], isnull([autoPay_cost], monthly_cost) as monthly_cost FROM [Plans] WHERE [planid] = @PlanId and isnull(active, 1) = 1"
        Else
            sql = "SELECT [planref], [plan_cost], [monthly_cost] FROM [Plans] WHERE [planid] = @PlanId and isnull(active, 1) = 1"
        End If
    ...
    Private Sub SaveRenwals(ByVal args As String, ByVal source As String)
        ' Response.Write("Called: " & DateTime.Now)
        'Response.End()
        UpdateLog(0, varOrderID, "BeforeRenew" & source)
        ...
        If rowsAffected = 1 Then
            ...
            sql.Append("SELECT [planref], [plan_cost], isnull(autopay_cost, [monthly_cost]) as monthly_cost FROM plans ")
        ...
        UpdateLog(0, varOrderID, "AfterRenew" & source)
    ...
    Protected Sub submitVerReqBtn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles submitVerReqBtn.Click
        ...
        If varCarrierName.ToLower <> "verizon" And varCarrierName.ToLower <> "concord" And varCarrierName.ToLower <> "telco" And varCarrierName.ToLower <> "sprint" Then
            Exit Sub
        End If
        ...
        ElseIf varCarrierName.ToLower = "concord" Then
            If esnChanged Then
                statusMsg = SubmitChangeConEsn()
            Else
                statusMsg = SubmitConcordReq()
            ...
            'ElseIf varCarrierName.ToLower = "sprint" Then
            '    If esnChanged Then
            '        statusMsg = SubmitChangeSprintEsn()
            '    Else
            '        statusMsg = SubmitSprintReq()

            '    End If

        ElseIf varCarrierName.ToLower = "telco" Then
            If esnChanged Then
                statusMsg = SubmitChangeTelcoEsn()
            Else
                statusMsg = SubmitTelcoReq()

            End If
        End If

        If statusMsg = "Error" Then
            Exit Sub
        End If
    ...
    Protected Sub executeAdminTasks_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles executeAdminTasks.Click
        ...
        displayRefundDiv.Visible = False



        'SG 07/13/2016 removed all talk
        ...
        '    End If 
        If adminTasksRadiolist.SelectedValue.ToLower = "disconnect" Then 'disconnect verizon
            cmd.Connection.Open()
            cmd.CommandType = CommandType.StoredProcedure
            ...
            ElseIf varCarrierName.ToLower = "telco" Then
                cmd.CommandText = "Telco_Disconnect"
            End If
            cmd.ExecuteNonQuery()
        ...
        ElseIf adminTasksRadiolist.SelectedValue.ToLower = "convertpp" Then 'convert from verizon / all talk to pp
            ...
            If varCarrierName.ToLower <> "verizon" Then
                ...
                'Else
                '    cmd.CommandText = "ATC_ConvertPP"  sg 07/13/16 removing all talk
            End If
            cmd.ExecuteNonQuery()
            'cmd.Connection.Close()

            'ElseIf adminTasksRadiolist.SelectedValue = "changeMDN" Then 'change Verizon MDN (not used) ' SG 12/21/16 removed
            '    If varCarrierName.ToLower = "verizon" Then
            '        cmd.Connection.Open()
            '        cmd.CommandType = CommandType.StoredProcedure
            '        cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
        ...
        ElseIf adminTasksRadiolist.SelectedValue.ToLower = "updatestack" Then 'update stack
            ...
            ElseIf varCarrierName.ToLower = "telco" Then
                cmd.CommandText = "Telco_ReleaseStack"
            End If
        ...
        ElseIf adminTasksRadiolist.SelectedValue.ToLower = "removestack" Then 'subtract one from stack
            ...
            '    ATCPin.ConvertAT(varOrderCellNumber, varEsn, varOrderID, statusMsg) 'as per email from Mr. Greenberg on 4.3.2014 SG 07/13/16 removed all talk

        ElseIf adminTasksRadiolist.SelectedValue.ToLower = "refund" Then
            reload = False

            displayRefundDiv.Visible = True
            Bindrefundtransgridview()

        ElseIf adminTasksRadiolist.SelectedValue.ToLower = "convertedtosprint" Then
            reload = False

            divConvertSprint.Visible = True


        ElseIf adminTasksRadiolist.SelectedValue = "convertedToCON" Then 'if converted to concord on the backend
            cmd.Connection.Open()
            'cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "update orders set carrier_name = 'Concord', statusmsg = 'Converted to Concord " + DateTime.Now.ToShortDateString + "' "
            cmd.CommandText = cmd.CommandText & "where cell_num = @mdn  "
            cmd.CommandText = cmd.CommandText & "if not exists (select * from conmdn where mid = @mdn) begin "
            cmd.CommandText = cmd.CommandText & "insert into conmdn (mid) values (@mdn) exec [run_con_query] @mdn end "
            cmd.Parameters.Add("@MDN", SqlDbType.VarChar).Value = varOrderCellNumber
        ...
        ElseIf adminTasksRadiolist.SelectedValue = "convertTELCO" Then 'if converted to Telco on the backend

            cmd.Connection.Open()
            'cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "update orders set carrier_name = 'Telco', statusmsg = 'Converted to Telco " + DateTime.Now.ToShortDateString + "' "
            cmd.CommandText = cmd.CommandText & "where cell_num = @mdn  "
            cmd.CommandText = cmd.CommandText & "if not exists (select * from telcomdn where mid = @mdn) begin "
            If varCarrierName.ToLower = "verizon" Then
                cmd.CommandText = cmd.CommandText & "exec Telco_CloneVerMDN '" & varOrderCellNumber & "' end " 'exec [run_con_query] @mid end "
            Else
                cmd.CommandText = cmd.CommandText & "insert into telcomdn (mid) values (@mdn) end "
            End If
        ...
        Else
            ...
            '    If customer.Address = "" Or customer.City = "" Or customer.State = "" Or customer.Zip = "" Or customer.FirstName = "" Or String.IsNullOrWhiteSpace(customer.FirstName) Then
            ...
            '        For Each c As Char In customer.Address
            ...
            '    ' Else
            '    'ElseIf task = "convertTELCO" Then
            '    '    cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
            '    '    cmd.Parameters.Add("@ESN", SqlDbType.VarChar).Value = varEsn
            '    '    cmd.Parameters.Add("@MDN", SqlDbType.VarChar).Value = varOrderCellNumber
            '    '    cmd.CommandText = "Telco_Transfer"
            '    '    statusMsg = "Submitted Convert Request"
            'Else
    ...
    End Sub
    Private Sub Bindrefundtransgridview()

        Dim sql As String = "SELECT [transid], [paydate], [monthly_amt], [cash_amt], [total], [user], [intl_amt], [item_amt], [agent], [authmessage], "
        sql &= "[trans_type] = CASE [trans_type]  WHEN 1 THEN 'Credit Card' WHEN 2 THEN 'Agent' ELSE '' End "
        sql &= "FROM [authtrans] WHERE [orderid] = " & varOrderID & " "
        sql &= "AND [charged] = 1 "
        sql &= "And paydate > dateadd(mm,-3,GetDate()) ORDER BY paydate desc"

        RefundDataSource.ConnectionString = ConfigurationManager.ConnectionStrings(_ppcConStr).ConnectionString
        RefundDataSource.SelectCommandType = SqlDataSourceCommandType.Text
        RefundDataSource.SelectCommand = sql
        RefundDataSource.CancelSelectOnNullParameter = False

        refundtransgridview.DataBind()
        'refundtransgridview.Columns(0).Visible = False

        If refundtransgridview.Rows.Count <= 0 Then
            refundInfoDiv.Visible = False
            errorLbl.ForeColor = Drawing.Color.Black
            errorLbl.Text = "There have been no payments on this account within the last 3 months."
            RefundBtn.Visible = False
        End If
        connection.Close()
    End Sub

    Protected Sub refundtransgridview_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs) Handles refundtransgridview.Sorting
        Bindrefundtransgridview()
    End Sub
    ...
    Private Function CreditCardCharge() As Charge
        ...
        'SG 01/05/17 if monthly on autopay - get autopay price does not apply for cash
        UpdateLog(0, varOrderID, "BeforePay")

        Dim autoPay_Monthly As Boolean = If(monthlyPlanDropdown.SelectedItem.Text.IndexOf("AutoPay") > -1, True, False)

        'Dim autoPay_Cash As Boolean = If(cashBalanceDropdown.SelectedItem.Text.IndexOf("auto") > -1 Or varIsCashRenew = True, True, False)
        ...
        ccCharge.MonthlyAmnt = GetCostForCharge(monthlyPlanDropdown.SelectedValue, autoPay_Monthly)
        ccCharge.CashAmnt = GetCostForCharge(cashBalanceDropdown.SelectedValue, False)
        ...
        If autoPay_Monthly = True Then

            Dim args As String = monthlyPlanDropdown.SelectedValue & "," & "true~0,false~0.00,false~1"

            SaveRenwals(args, "Pay")
        End If
        UpdateLog(ccCharge.TransactionId, ccCharge.OrderId, "AfterPay")
        Return ccCharge
    ...
    Private Function AgentAccountCharge() As Charge
        ...
        Dim autoPay_Monthly As Boolean = If(monthlyPlanDropdown.SelectedItem.Text.IndexOf("AutoPay") > -1, True, False)
        'Dim autoPay_Cash As Boolean = If(cashBalanceDropdown.SelectedItem.Text.IndexOf("auto") > -1 Or varIsCashRenew = True, True, False)
        ' Response.Write(autoPay_Monthly)
        'Response.End()
        ...
        agentCharge.MonthlyAmnt = GetCostForCharge(monthlyPlanDropdown.SelectedValue, autoPay_Monthly)
        agentCharge.CashAmnt = GetCostForCharge(cashBalanceDropdown.SelectedValue, False)
        ...
        If autoPay_Monthly = True Then
            Dim args As String = monthlyPlanDropdown.SelectedValue & "," & "true~0,false~0.00,false~2"
            SaveRenwals(args, "Pay")
        End If

        UpdateLog(agentCharge.TransactionId, agentCharge.OrderId, "AfterPay")

        Return agentCharge

    End Function

    Protected Sub UpdateLog(ByVal transid As Integer, ByVal orderid As Integer, ByVal source As String)
        ...
        cmd.CommandText = "insert into autorenewlog "
        cmd.CommandText = cmd.CommandText & "select " & orderid & ", " & transid & ", monthly_auto_renew, monthly_plan_id, cash_auto_renew,  o.cash_plan_id, renew_charge_type, "
        cmd.CommandText = cmd.CommandText & "getdate(), plan_id, carrier_name, total, month_plan_id, t.cash_plan_id, '" & source & "' "
        cmd.CommandText = cmd.CommandText & "from orders o left join authtrans t on o.order_id = t.orderid and t.transid = "
        If transid <> 0 Then
            cmd.CommandText = cmd.CommandText & transid
        Else
            cmd.CommandText = cmd.CommandText & " (select top 1 transid from authtrans where orderid = " & orderid & " and charged = 1 order by paydate desc)"
        End If

        cmd.CommandText = cmd.CommandText & " where order_id = " & orderid
    ...
    Private Sub SetupNumberOfCallDetailsIntervals()
        If varCarrierName.ToLower = "verizon" Or varCarrierName.ToLower = "telco" Then
            GetVerCDRIntervals()
        ElseIf varCarrierName.ToLower = "page plus" Or varCarrierName.ToLower = "vzpp" Then
            For Each tab As AjaxControlToolkit.TabPanel In TabContainer1.Tabs()
    ...
    Private Function CreateOrder() As Boolean
        ...
        sql.Append("[intl_pin], [carrier_name], [carrier_pin], [vendor_pin], [monitor] , [statusmsg]) ")
        ...
        sql.Append("@intl_pin, @carrier_name, @carrierpin, @vendor_pin, @monitor, @msg ); ")


        If carrierName.SelectedValue.ToLower = "vzpp" Then

            sql.Append("exec vzpp_createMDN '" & cell_number.Text & "', ")
            If IsDate(tbVZPPPlanStart.Text) Then
                sql.Append("'" & DateTime.Parse(tbVZPPPlanStart.Text) & "' ")
            Else
                sql.Append("null ")
            End If
        End If
        ...
        cmd.Parameters.Add("@cell_number", SqlDbType.VarChar).Value = StrOrDBNull(
            If(carrierName.SelectedValue.ToLower = "page plus" Or carrierName.SelectedValue.ToLower = "vzpp", cell_number.Text, "")) 'Or carrierName.SelectedValue.ToLower = "all talk"
        ...
        If carrierName.SelectedValue.ToLower = "page plus" Then
            ...
            cmd.Parameters.Add("@carrierpin", SqlDBType.Varchar).Value = DBNull.Value
        ElseIf CarrierName.SelectedValue.ToLower = "vzpp" Then
            cmd.Parameters.Add("@vendor_pin", SqlDbType.VarChar).Value = StrOrDBNull(vzppVendorPin.Text)
            cmd.Parameters.Add("@carrierpin", SqlDBType.Varchar).Value = StrOrDBNull(vzppVendorPW.Text)
        Else
            cmd.Parameters.Add("@carrierpin", SqlDBType.Varchar).Value = DBNull.Value
            cmd.Parameters.Add("@vendor_pin", SqlDbType.VarChar).Value = DBNull.Value
                End If

                If carrierName.SelectedValue.ToLower = "sprint" Then
                    cmd.Parameters.Add("@msg", SqlDBType.Varchar).Value = "New Service will complete once a plan is purchased."

                Else
                    cmd.Parameters.Add("@msg", SqlDBType.Varchar).Value = DBNull.Value
                End If
                ...
                'End If
                ''If carrierName.SelectedValue = "Concord" Then
                ''    Session("esnError") = "Port will complete once a plan is purchased."
                ''End If
    ...
    Private Function SubmitVerReq() As String
        ...
        If customer.Address = "" Or customer.City = "" Or customer.State = "" Or customer.Zip = "" Or customer.FirstName = "" Or String.IsNullOrWhiteSpace(customer.FirstName) Then
            toggleOrderErrorMsg.InnerHtml = "Please update customer information to include first name, address, city, state, and zip."
            statusMsg = "Error"
        ElseIf customer.State.Length > 2 Then
            toggleOrderErrorMsg.InnerHtml = "Please use the 2 letter abbreviation for the State."
            statusMsg = "Error"
        ElseIf customer.Address.Length > 1 Then
            For Each c As Char In customer.Address
                If Char.IsSymbol(c) Then
                    toggleOrderErrorMsg.InnerHtml = "Please remove any special charecters from the customer's address."
                    statusMsg = "Error"
                    Exit For
                End If
            Next
    ...
    Private Function SubmitChangeConEsn() As String
        Dim cmd As New SqlCommand
        ...
        Return "Submitted Concord Change ESN Request"

    End Function

    Private Function SubmitChangeTelcoEsn() As String
        Dim cmd As New SqlCommand
        ...
        cmd.CommandText = "Telco_ChangeESN"
        cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
        cmd.Parameters.Add("@MDN", SqlDbType.VarChar).Value = varOrderCellNumber
        cmd.Parameters.Add("@NEWESN", SqlDbType.VarChar).Value = hiddenESN.Value
        ...
        Return "Submitted Telco Change ESN Request"
    End Function


    Private Function SubmitChangeSprintEsn() As String
        Dim cmd As New SqlCommand
        ...
        cmd.CommandText = "SprintChangeESN"
        cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
        cmd.Parameters.Add("@MDN", SqlDbType.VarChar).Value = varOrderCellNumber
        cmd.Parameters.Add("@NEWESN", SqlDbType.VarChar).Value = hiddenESN.Value
        ...
        Return "Submitted Sprint Change ESN Request"

    End Function

    Private Function SubmitConcordReq() As String
        ...
        If newNumberRadioType.SelectedValue.ToLower = "new" Then
            hiddenZip.Value = IIf(createZipNPARadioList.SelectedValue = "createPhoneZip", createPhoneZip.Text, String.Concat(createPhoneNPA.Text, createPhoneNXX.Text))

            If hiddenZip.Value.Length = 0 Then
                toggleOrderErrorMsg.InnerHtml = "Please enter a valid Zip or NPA - NXX code."
                statusMsg = "Error"
            End If
            'cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "Update orders set statusmsg='NewService' where order_id = @orderid update customers set npazip = @npazip where customer_id = " & varCustomerId & " "
            cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
            cmd.Parameters.AddWithValue("@npazip", hiddenZip.Value)



            statusMsg = "New service will activate once plan is purchased"

        ElseIf newNumberRadioType.SelectedValue.ToLower = "port" Then
            If Not IsCellNumberUnique(portPhone.Text.Trim) Then
                cmd.CommandText = " Update orders set cell_num = stuff(@cell,1,1,'0'), order_closed = 1 where cell_num = @cell "
                toggleOrderErrorMsg.InnerHtml = "The phone number exists on another account. Closed old account."
                'statusMsg = "Error"
                'Else 
            End If
            cmd.CommandText = cmd.CommandText & " Update orders set statusmsg='PortService', cell_num = @cell where order_id = @orderid "
            cmd.CommandText = cmd.CommandText & " Update customers set portaccountnumber = '" & carrierCodeFld.Text.Trim & "', portpassword = '" & portPwFld.Text.Trim & "' where customer_id = " & varCustomerId & " "
            cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
            cmd.Parameters.AddWithValue("@cell", portPhone.Text.Trim)
            statusMsg = "Port will complete once plan is purchased."
        ...
        ElseIf customer.State.Length > 2 Then
            toggleOrderErrorMsg.InnerHtml = "Please use the 2 letter abbreviation for the State."
            statusMsg = "Error"
        ElseIf customer.FirstName.Length < 2 Then
            toggleOrderErrorMsg.InnerHtml = "Customer's first name must be longer then 1 character"
            statusMsg = "Error"
        ElseIf customer.Address.Length > 1 Then
            For Each c As Char In customer.Address
                If Char.IsSymbol(c) Then
                    toggleOrderErrorMsg.InnerHtml = "Please remove any special characters from the customer's address."
                    statusMsg = "Error"
                    Exit For
                End If
            Next
    ...
    Private Function SubmitTelcoReq() As String
        Dim cmd As New SqlCommand
        Dim statusMsg As String = Nothing
        ...
        If newNumberRadioType.SelectedValue.ToLower = "new" Then
            ...
            ' Response.Write(hiddenZip.Value)
            'Response.End()
            statusMsg = "New service will activate once plan is purchased."
        ...
        ElseIf newNumberRadioType.SelectedValue.ToLower = "port" Then
            ...
            cmd.CommandText = cmd.CommandText & " Update orders set statusmsg='NewPortService', cell_num = @cell where order_id = @orderid "
            cmd.CommandText = cmd.CommandText & " Update customers set portaccountnumber = '" & carrierCodeFld.Text.Trim & "', portpassword = '" & portPwFld.Text.Trim & "' where customer_id = " & varCustomerId & " "
            'Response.Write(cmd.CommandText)
            'Response.End()
        ...
        'If newNumberRadioType.SelectedValue.ToLower = "new" Then
        '    cmd.CommandType = CommandType.StoredProcedure
        '    cmd.CommandText = "Telco_NewService"
        '    cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
        '    cmd.Parameters.Add("@ESN", SqlDbType.VarChar).Value = hiddenESN.Value
        '    cmd.Parameters.Add("@NpaZip", SqlDbType.VarChar).Value =
        '        If(createZipNPARadioList.SelectedValue = "createPhoneZip", createPhoneZip.Text,
        '            String.Concat(createPhoneNPA.Text, createPhoneNXX.Text))
        '    statusMsg = "Submitted Telco New Service Request"
        'ElseIf newNumberRadioType.SelectedValue.ToLower = "port" Then
        '    cmd.CommandText = "Telco_NewServicePort"
        '    cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
        '    cmd.Parameters.Add("@MDN", SqlDbType.VarChar).Value = portPhone.Text.Trim
        '    cmd.Parameters.Add("@PORTPASSWD", SqlDbType.VarChar).Value = portPwFld.Text.Trim
        '    cmd.Parameters.Add("@ESN", SqlDbType.VarChar).Value = hiddenESN.Value
        '    cmd.Parameters.Add("@PORTTYPE", SqlDbType.VarChar).Value = phoneOriginTypeRadio.SelectedValue
        '    cmd.Parameters.Add("@PORTACCT", SqlDbType.VarChar).Value = carrierCodeFld.Text
        '    statusMsg = "Submitted Telco New Service Port Request"
        'End If
    ...
    Private Function SubmitSprintReq() As String
        ...
        If newNumberRadioType.SelectedValue.ToLower = "new" Then
            ...
            If hiddenZip.Value.Length = 0 Then
                toggleOrderErrorMsg.InnerHtml = "Please enter a valid Zip Code."
                statusMsg = "Error"
            End If
            'cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "Update orders set statusmsg='NewService' where order_id = @orderid update customers set npazip = @npazip where customer_id = " & varCustomerId & " "
    ...
    Private Sub UpdateOrder()
        Dim changedCarrier As Boolean = False
        Dim newSprintNumber As Boolean = False
        If varCarrierName <> carrierName.SelectedValue Then
            changedCarrier = True
        End If

        If varCarrierName.ToLower = "sprint" And String.IsNullOrEmpty(varOrderCellNumber) And txtCellNum.Visible Then
            newSprintNumber = True
        End If
        ...
        If carrierName.SelectedValue.ToLower = "page plus" Or carrierName.SelectedValue.ToLower = "vzpp" Or carrierName.SelectedValue.ToLower = "sprint" Then
            ...
            '        End If
            '    End If
        ElseIf carrierName.SelectedValue.ToLower = "verizon" Or carrierName.SelectedValue.ToLower = "telco" Then
            If (serialNumber.Text = varSerialNumber) Then
                sql.Append("[serial_num] = @serial_number, [esn] = @esn,  ")
            End If

        ElseIf carrierName.SelectedValue.ToLower = "concord" And changedCarrier = True Then
            sql.Append("[statusmsg] = 'PortService',  ")
        End If

        sql.Append("[intl_pin] = @intl_pin, [carrier_name] = @carrier_name, ")
        sql.Append("[vendor_pin] = @vendor_pin, [carrier_pin] = @carrierpin, [monitor] = @monitor ")
        sql.Append("WHERE [order_id] = @OrderID ")


        If newSprintNumber = True Then
            sql.Append("exec createSprintMDN @orderid, @cell_number ")
        End If

        If carrierName.SelectedValue.ToLower = "vzpp" Then 'changedCarrier = True And 
            sql.Append("exec vzpp_createMDN '" & cell_number.Text & "', ")
            If IsDate(tbVZPPPlanStart.Text) Then
                sql.Append("'" & DateTime.Parse(tbVZPPPlanStart.Text) & "' ")
            Else
        ...
        If (carrierName.SelectedValue = "Concord" Or carrierName.SelectedValue = "Telco" Or carrierName.SelectedValue = "Sprint") And changedCarrier = True Then
            Session("esnError") = "Port will complete once a plan is purchased."
        End If
    ...
    Private Sub ChargeOrder()
        ...
        End If
        'Response.Write(" Charged: " & charge.hasCharged)
        ...
        If Not charge.hasCharged Then
            ...
            'Response.Write(charge.AuthMessage)
        ...
        Else
            ...
            'Moved her to ensure payment went thru : 10/21/14
            If varCarrierName.ToLower = "verizon" _
               And (monthlyPlanDropdown.SelectedValue > 0 Or cashBalanceDropdown.SelectedValue > 0) Then
                UpdateVerMdn()
            ElseIf varCarrierName.ToLower = "concord" Then
                UpdateConMdn(PlanId, charge.TransactionId)
            ElseIf varCarrierName.ToLower = "telco" And (monthlyPlanDropdown.SelectedValue > 0 Or cashBalanceDropdown.SelectedValue > 0) Then
                UpdateTelcoMdn(PlanId, charge.TransactionId)
            ElseIf varCarrierName.ToLower = "vzpp" Then
                UpdateVZPP(monthlyId, charge.TransactionId) 'sg 04/20/17 changed to use the month plan id to exclude intl and item payments **if cash plan is added to carrier this wont work**
            ElseIf varCarrierName.ToLower = "sprint" Then
                UpdateSprintMdn(PlanId, charge.TransactionId)
            End If
    ...
    Private Sub UpdateConMdn(ByVal planid As Integer, ByVal transid As Integer)  'SG 10/11/16

        Dim cmd As New SqlCommand
        cmd.Connection = New SqlConnection(ConfigurationManager.ConnectionStrings(_ppcConStr).ConnectionString)
        cmd.CommandType = CommandType.StoredProcedure

        If varOrderCellNumber.Length > 1 Then
            cmd.CommandText = "ConcordNewPayment"
            cmd.Parameters.Add("@orderid", SqlDbType.Int).Value = varOrderID
            cmd.Parameters.Add("@cellnum", SqlDbType.VarChar).Value = varOrderCellNumber
        ...
        Else
            cmd.CommandText = "ConcordNewCust"
            cmd.Parameters.Add("@orderid", SqlDbType.Int).Value = varOrderID
            cmd.Parameters.Add("@npazip", SqlDbType.VarChar).Value = hiddenZip.Value
            cmd.Parameters.AddWithValue("@planid", planid)
            cmd.Parameters.Add("@transid", SqlDbType.Int).Value = transid
        End If
    ...
    Private Sub UpdateTelcoMdn(ByVal planid As Integer, ByVal transid As Integer)  'SG 10/11/16
        ...
        'If varOrderCellNumber.Length > 1 Then
        cmd.CommandText = "Telco_AllocatePayment"
        cmd.Parameters.Add("@orderid", SqlDbType.Int).Value = varOrderID
        cmd.Parameters.Add("@cellnum", SqlDbType.VarChar).Value = varOrderCellNumber
        cmd.Parameters.AddWithValue("@planid", planid)
        cmd.Parameters.Add("@npazip", SqlDbType.VarChar).Value = hiddenZip.Value
    ...
    Private Sub UpdateSprintMdn(ByVal planid As Integer, ByVal transid As Integer)  'SG 10/11/16
        'Response.Write(planid & " - " & transid)
        Dim cmd As New SqlCommand
        cmd.Connection = New SqlConnection(ConfigurationManager.ConnectionStrings(_ppcConStr).ConnectionString)
        cmd.CommandType = CommandType.StoredProcedure

        'If varOrderCellNumber.Length > 1 Then
        cmd.CommandText = "SprintNewPayment"
        cmd.Parameters.Add("@orderid", SqlDbType.Int).Value = varOrderID
        'cmd.Parameters.Add("@cellnum", SqlDbType.VarChar).Value = varOrderCellNumber
        cmd.Parameters.AddWithValue("@planid", planid)
        'cmd.Parameters.Add("@npazip", SqlDbType.VarChar).Value = hiddenZip.Value
    ...
    Private Sub UpdateVZPP(ByVal planId As Integer, ByVal transid As Integer)
        'Response.Write(planId & " " & transid & " " & varOrderCellNumber)
        'Response.End()
        Dim cmd As New SqlCommand
        cmd.Connection = New SqlConnection(ConfigurationManager.ConnectionStrings(_ppcConStr).ConnectionString)
        cmd.CommandType = CommandType.StoredProcedure

        cmd.CommandText = "VZPP_Pay"
        cmd.Parameters.AddWithValue("@cellnum", varOrderCellNumber)
        cmd.Parameters.AddWithValue("@monthplanid", planId)
    ...
    Private Sub InsertCommission(ByVal transId As Integer)
        Dim cmd As New SqlCommand
        cmd.Connection =
            New SqlConnection(ConfigurationManager.ConnectionStrings(_ppcConStr).ConnectionString) 'GetVerizonCdr
    ...
    Public Sub RaiseCallbackEvent(ByVal eventArgument As String) _
     Implements System.Web.UI.ICallbackEventHandler.RaiseCallbackEvent
        ...
        If getAction = "SetCostLblFromServer" Then
            Dim info As String() = eventArgument.Split(":"c)
            Dim planId As String = info(1)
            Dim monthlyAutoPay As Boolean = If(info(2).IndexOf("Auto") > -1, True, False)
            'Dim planId As String = eventArgument.Substring(InStr(eventArgument, ":"))
            GetPlanCost(planId, True, monthlyAutoPay)
        End If
    ...
    Protected Sub statusLkBn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles statusLkBn.Click
        ...
        ElseIf varCarrierName.ToLower = "concord" Then
            ...
            cmd.CommandText = "run_con_query"
            cmd.Parameters.Add("@mid", SqlDbType.VarChar).Value = varOrderCellNumber
    ...
    Protected Sub refundtransgridview_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles refundtransgridview.RowDataBound
        'If e.Row.RowState = DataControlRowState.Normal Then


        If e.Row.RowType = DataControlRowType.DataRow Then
            'Response.Write(refundtransgridview.DataKeys.Count)
            'Response.End()
            'Dim rowId As String = refundtransgridview.DataKeys(e.Row.RowIndex).Value(0).ToString() ' e.Row.Cells(0).Text.ToString()

            Dim amount As String = e.Row.Cells(9).Text.ToString()
            Dim row As DataRow = DirectCast(e.Row.DataItem, DataRowView).Row


            'e.Row.Attributes.Add("onclick", "setClickBGColor(this);")
            e.Row.Attributes.Add("onclick", "showCharge('" & amount & "','" & e.Row.DataItem("transid") & "'); setClickBGColor(this);")

        End If

        'End If
        Label1.Text = Date.Today.ToShortDateString()
        'errorLbl.InnerText = ""

    End Sub
    Protected Sub RefundBtn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RefundBtn.Click

        Dim refCharge As New CreditCardCharge

        If transIdHF.Value.Length <= 0 Then

            errorLbl.ForeColor = Drawing.Color.Red
            errorLbl.Text = "Please select a payment from the GridView"
            Exit Sub
        Else
            errorLbl.Text = ""
        End If

        Dim transid As Integer = Integer.Parse(transIdHF.Value)

        refCharge.varTotal = Decimal.Parse(rfAmount.Text, NumberStyles.Currency)
        refCharge.CCNumber = creditCardNumber.Text
        refCharge.CellNumber = varOrderCellNumber
        refCharge.OrderId = varOrderID
        'Response.Write(refCharge.varTotal)
        'errorLbl.InnerText = refCharge.varTotal.ToString()
        refCharge.RefundTrans(transid)

        errorLbl.ForeColor = Drawing.Color.Black
        errorLbl.Text = refCharge.AuthMessage

    End Sub

    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        If txtSprintCustId.Text.Length = 0 Then
            Exit Sub
        End If
        ...
        cmd.Connection.Open()
        'cmd.CommandType = CommandType.StoredProcedure
        cmd.CommandText = "update orders set carrier_name = 'Sprint', statusmsg = 'Converted to Sprint " + DateTime.Now.ToShortDateString + "' "
        cmd.CommandText = cmd.CommandText & "where cell_num = @mdn  "
        cmd.CommandText = cmd.CommandText & "if not exists (select * from sprintmdn where mid = @mdn) begin "

        cmd.CommandText = cmd.CommandText & "insert into sprintmdn (mid, sprintcode) values (@mdn, @sprintCustId) end "


        cmd.Parameters.Add("@MDN", SqlDbType.VarChar).Value = varOrderCellNumber
        cmd.Parameters.Add("@sprintCustId", SqlDbType.VarChar).Value = txtSprintCustId.Text
