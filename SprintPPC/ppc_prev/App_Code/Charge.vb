...
Public MustInherit Class Charge
    ...
    Protected Function SetRefundAuthErrorMessage(ByRef AuthMsg As String, ByRef transid As Int32) As Boolean

        Dim con As SqlConnection =
            New SqlConnection(ConfigurationManager.ConnectionStrings("ppcConnectionString").ConnectionString)
        con.Open()

        Dim failedAuthSql As String = "UPDATE [authrefundtrans] SET [auth_message] = '" & AuthMsg & "' WHERE [ref_id] = " & transid
        Dim failedAuthCmd = New SqlCommand(failedAuthSql, con)
