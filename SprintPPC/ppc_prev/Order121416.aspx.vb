...
Partial Class Order
    ...
    Private Sub PopulateAllPlanDropdowns()
        ...
        Dim sql As String = "SELECT [planid], [planname] FROM [Plans] WHERE planref = 'Monthly' AND [carrier] = @carrier ORDER BY [minutesprovided]"
        cmd = New SqlCommand(sql, connection)
    ...
    Private Sub GetTelcoCdr()
        ...
        ' Get cycle dates and total minutes
        sql.Append("SELECT @Start AS Start, @End AS [End], SUM( CALLDURMIN ) AS TotalMinutes FROM [Telco].[dbo].CDR_NEW ")
        sql.Append("WHERE [BILLING NUMBER] = @CellNum  AND [CALLTIME] BETWEEN @Start AND @End; ")
    ...
    Protected Sub executeAdminTasks_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles executeAdminTasks.Click
        ...
        ElseIf adminTasksRadiolist.SelectedValue = "changeMDN" Then 'change Verizon MDN (not used) ?????? DO WE NEED THIS FOR TELCO SG 12/13/16
            If varCarrierName.ToLower = "verizon" Then
                cmd.Connection.Open()
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
        ...
        ElseIf adminTasksRadiolist.SelectedValue = "convertTELCO" Then 'if converted to Telco on the backend
            ...
            cmd.CommandText = cmd.CommandText & "insert into telcomdn (mid) values (@mdn) end" 'exec [run_con_query] @mdn end "
            cmd.Parameters.Add("@MDN", SqlDbType.VarChar).Value = varOrderCellNumber
