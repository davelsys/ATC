...
Calendar.showYearsCombo=function(fwd)
{
   ...
   if(show)
   {
        var s=yc.style;s.display="block";
 ...
 Calendar.tableMouseUp=function(ev)
 {
    ...
    if(mon){
        ...
        if(mon.month!=date.getMonth()){
            ...
            cal.setDate(date);cal.dateClicked=false;
            cal.callHandler();
        }
    }
    else{
...
Calendar.dayMouseDown=function(ev)
{
    ...
    var cal=el.calendar;cal.activeDiv=el;
    Calendar._C=cal;
    if(el.navtype!=300)
        with(Calendar){
            if(el.navtype==50){
...
Calendar.cellClick=function(el,ev)
{
    ...
    if(typeof el.navtype=="undefined"){
        ...
        switch(el.navtype){
            case 400:   Calendar.removeClass(el,"hilite");
           ...
           case 0:     if((typeof cal.getDateStatus=="function")&&cal.getDateStatus(date,date.getFullYear(),date.getMonth(),date.getDate()))
                        {
                            return false;
                        }
                        break;
...
Calendar.prototype.create=function(_par)
{
    ...
    cell=hh(Calendar._TT["SEL_DATE"],
    this.weekNumbers?8:7,300);
    cell.className="ttip";
    if(this.isPopup){
...
Calendar._keyEvent=function(ev)
{
    ...
    {
        switch(K){
            case 37:act&&Calendar.cellClick(cal._nav_pm);
                    break;
        ...
        switch(K){
            case 32:Calendar.cellClick(cal._nav_now);
                 break;
                            ...
                            switch(K){
                                ...
                                case 38:if(--y>=0)
                                 ...
                                 case 39:if(++x<7)
                                         ...
                                         break;
                                  case 40:if(++y<cal.ar_days.length)
                                            ne=cal.ar_days[y][x];
                                          else{
                                            nextMonth();
                                            setVars();
...
{
    this._init(firstDayOfWeek,this.date);this._displayWeekdays();
};
...
Calendar.prototype.show=function()
{
    ...
    {
        var row=rows[--i];Calendar.removeClass(row,"rowhilite");
...
Calendar.prototype.showAtElement=function(el,opts)
{
   ...
   Calendar.prototype._displayWeekdays=function()
   {
        ...
        for(var i=0;i<7;++i)
        {
            ...
            cell.navtype=100;cell.calendar=this;
            cell.fdow=realday;Calendar._add_evs(cell);
        }
...
Calendar.prototype._dragStart=function(ev)
{
    ...
    if(Calendar.is_ie)
    {
        ...
        posY=ev.clientY+window.scrollY;posX=ev.clientX+window.scrollX;
    }
...
Date.parseDate=function(str,fmt)
{
    ...
    if(y!=0&&m!=-1&&d!=0)
        return new Date(y,m,d,hr,min,0);y=0;m=-1;d=0;
...
Date.prototype.getDayOfYear=function()
{
    ...
    var time=now-then;return Math.floor(time/Date.DAY);
};
...
Date.prototype.print=function(str)
{
    ...
    s["%p"]=pm?"PM":"AM";
    s["%P"]=pm?"pm":"am";
    s["%s"]=Math.floor(this.getTime()/1000);s["%S"]=(sec<10)?("0"+sec):sec;
    s["%t"]="\t";
