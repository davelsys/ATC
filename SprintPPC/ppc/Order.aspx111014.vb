...
Partial Class Order
    ...
    Public Sub GetVerizonCdr()
        ...
        sql.Append("WHERE MID = @CellNum and [CALL DIRN IND] != '5' AND [CALLTIME] BETWEEN @Start AND @End; ")
    ...
    Protected Sub executeAdminTasks_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles executeAdminTasks.Click
        ...
        Else
            ...
            If task = "convertver" Then
                'if required fields are missing, set validreq = false
                If customer.Address = "" Or customer.City = "" Or customer.State = "" Or customer.Zip = "" Or customer.FirstName = "" Or customer.State.Length > 2 Then

                    validReq = False
                Else
                    customer.State = customer.State.ToUpper
                End If
            ...
            'if customer has all required fields-address, city, state, zip, (meaning validReq=true) then continue
            If validReq Then
                Dim sql As New StringBuilder
