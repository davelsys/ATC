...
Class SprintResp
    ...
    Private Sub BindSprintRespGrid()
        ...
        sql.Append("select Sprintreqid,Processed,ReqType,MSID,MSL,MDN,ESN,reqStatus,ISNULL(NullIF(RespStatus,''),reqstatus)[RespStatus],left(isnull(NullIF(RespAckMsg,''),ReqAckMsg),20)/*LTRIM(RTRIM(isnull(NullIF(RespAckMsg,''),ReqAckMsg)))*/[RespAckMsg],")
        sql.Append(" Poll,PollCount,lastpoll ")
        sql.Append(" from Sprint.dbo.Sprintreq where 1 = 1 ")
        ...
        Dim selectedPollRadio As String = pollSprintRadioList.SelectedValue
        ...
        Dim selectedRadio As String = filterSprintRadioList.SelectedValue
        ...
        sql.Append("ORDER BY sprintreqid DESC ")

        SprintRespGridviewDataSource.SelectCommandType = SqlDataSourceCommandType.Text
        SprintRespGridviewDataSource.SelectCommand = sql.ToString()
        SprintRespGridviewDataSource.CancelSelectOnNullParameter = False

        SprintRespGridview.DataBind()

    End Sub


    Protected Sub SprintRespGridview_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles SprintRespGridview.RowDataBound
    ...
    Protected Sub badTransDropdown_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim theid As Integer = Integer.Parse(SprintRespGridview.DataKeys(rowIndex).Value.ToString)
        ...
        If action = "RePoll" Then
            ...
            strSql = "UPDATE Sprint.dbo.Sprintreq SET [poll] = 1, pollcount = 0 WHERE [sprintreqid] = @Sprintid "
            ...
            cmd.Parameters.Add("@SprintId", SqlDbType.Int).Value = theid
            cmd.ExecuteNonQuery()
            con.Close()
        ...
        If action = "ReSubmit" Then
            ...
            strSql = "Exec Sprint_ReSubmit " & theid '.ToString()
        ...
        If action = "Clear" Then
            ...
            strSql = "UPDATE Sprint.dbo.Sprintreq SET [processed] = GetDate(),poll=0 WHERE [sprintreqid] = @SprintId "
    ...
    Protected Sub SprintRespGridview_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles SprintRespGridview.PageIndexChanging
        SprintRespGridview.PageIndex = e.NewPageIndex
        BindSprintRespGrid()
    End Sub

    Protected Sub SprintRespGridview_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs) Handles SprintRespGridview.Sorting
        BindSprintRespGrid()
    End Sub

    Protected Sub pollSprintRadioList_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles pollSprintRadioList.SelectedIndexChanged
        BindSprintRespGrid()
    End Sub
    Protected Sub filterSprintRadioList_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles filterSprintRadioList.SelectedIndexChanged
        BindSprintRespGrid()
    End Sub
