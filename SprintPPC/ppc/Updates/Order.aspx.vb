...
Partial Class Order
    ...
    Private Sub PopulateAllPlanDropdowns()
        ...
        Dim sql As String = "SELECT [planid], [planname] FROM [Plans] WHERE planref = 'Monthly' AND [carrier] = @carrier and isnull(active, 1) = 1 " 'ORDER BY [minutesprovided]"
        If varPaymentsExists = False Then
        ...
        sql = "SELECT [planid], [planname] FROM [Plans] WHERE planref = 'Pay As You Go' AND [carrier] = @carrier and isnull(active, 1) = 1"
    ...
    Private Sub GetConcordData()
        ...
        sql.Append("CAST( MonthExpDate AS DATE ) AS [MonthExpDate], ")
        sql.Append(" [MinutesinPlan], ")
        sql.Append(" case when conmdn.planid = 30 then (5000 - minutesavailable) else 0 end AS [min_remain], ")
        sql.Append("MinutesUpdated [LastStatus], [Stacked], [cashstacked],")
        sql.Append("MinutesAvailable [TotalMinutes], ")
        ' VerRegister
        'sql.Append("CAST( PlanExpDate AS DATE ) AS [PlanExpDate], ")
    ...
    Private Function GetPlanCost(ByVal planId As Integer, Optional ByVal isAjax As Boolean = False, Optional ByVal AutoPayPrice As Boolean = False) As Decimal
        ...
        If AutoPayPrice = True And (varCarrierName = "Telco" Or varCarrierName = "Concord") Then
            sql = "SELECT [planref], [plan_cost], [autoPay_cost] as monthly_cost FROM [Plans] WHERE [planid] = @PlanId and isnull(active, 1) = 1"
        Else
    ...
    Protected Sub submitVerReqBtn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles submitVerReqBtn.Click
        ...
        If varCarrierName.ToLower <> "verizon" And varCarrierName.ToLower <> "concord" And varCarrierName.ToLower <> "telco" Then
            Exit Sub
        End If
    ...
    Private Function CreateOrder() As Boolean
        ...
        cmd.Parameters.Add("@cell_number", SqlDbType.VarChar).Value = StrOrDBNull(
            If(carrierName.SelectedValue.ToLower = "page plus", cell_number.Text, "")) 'Or carrierName.SelectedValue.ToLower = "all talk"
    ...
    Private Function SubmitConcordReq() As String
        ...
        If newNumberRadioType.SelectedValue.ToLower = "new" Then
            'cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "Update orders set statusmsg='NewService' where order_id = @orderid "
            cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
        ...
        ElseIf newNumberRadioType.SelectedValue.ToLower = "port" Then
            cmd.CommandText = "Update orders set statusmsg='PortService', cell_num = @cell where order_id = @orderid "
            cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
    ...
    Private Function SubmitTelcoReq() As String
        ...
        ElseIf newNumberRadioType.SelectedValue.ToLower = "port" Then
            cmd.CommandText = "Update orders set statusmsg='NewPortService', cell_num = @cell where order_id = @orderid "
            cmd.Parameters.Add("@OrderId", SqlDbType.Int).Value = varOrderID
            cmd.Parameters.AddWithValue("@cell", portPhone.Text.Trim)
            statusMsg = "Port will complete once plan is purchased."
        End If
    ...
    Private Sub UpdateOrder()
        ...
        If (carrierName.SelectedValue = "Concord" Or carrierName.SelectedValue = "Telco") And changedCarrier = True Then
            Session("esnError") = "Port will complete once a plan is purchased."
        End If
