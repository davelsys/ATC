    ...
    Sub Main(ByVal args() As String)
        ...
        While rdr.Read()
            ...
            End If
              ...
              'Process Sprint
            If carrier.ToLower() = "sprint" And (cc.hasCharged Or agt.hasCharged) Then

                Dim conat As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("ppcConnectionString").ConnectionString)
                conat.Open()

                Dim sprint_renew_sp As New SqlCommand("exec sprint_AssignPayment @cellnum,@planid,@transid", conat)
                Try
                    If agt.hasCharged Then
                        sprint_renew_sp.Parameters.Add("@cellnum", SqlDbType.VarChar).Value = agt.CellNumber
                        sprint_renew_sp.Parameters.Add("@planid", SqlDbType.Int).Value = agt.MonthlyPlanId
                        sprint_renew_sp.Parameters.Add("@transid", SqlDbType.Int).Value = agt.TransactionId
                    ElseIf cc.hasCharged Then
                        sprint_renew_sp.Parameters.Add("@cellnum", SqlDbType.VarChar).Value = cc.CellNumber
                        sprint_renew_sp.Parameters.Add("@planid", SqlDbType.Int).Value = cc.MonthlyPlanId
                        sprint_renew_sp.Parameters.Add("@transid", SqlDbType.Int).Value = cc.TransactionId
                    End If

                    sprint_renew_sp.CommandTimeout = 64000
                    Console.WriteLine(String.Format("SP Sprint_AssignPayment .. (Timeout = {0})", sprint_renew_sp.CommandTimeout))
                    sprint_renew_sp.ExecuteNonQuery()
                Catch ex As Exception
                    Console.WriteLine("Error running Sprint_AssignPayment -" + ex.Message.ToString())
                End Try
